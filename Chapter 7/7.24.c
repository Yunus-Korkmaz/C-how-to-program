/*
	Name:         Mazes of Any Size
	Author:       Yunus Korkmaz
	Date:		  
	Description:  Generalize functions mazeTraverse and mazeGenerator of
...
void left_hand_rule(char *maze[],int x,int y,direction_list* d1);
direction_list* turn_left(char *maze[],int* x,int* y,direction_list* d1);
int go_straight(char *maze[],int* x,int* y,direction_list* d1);
direction_list* turn_right(char *maze[],int* x,int* y,direction_list* d1);
...
int main(void){
	...
	printf("Pls enter zie of array X * Y (x and y need to be grater than 4) : \n");
	printf("size of y = ");
	scanf("%d",&size_y);
	printf("size of x= ");
	scanf("%d",&size_x);
	...
	for(i=0;i<size_y;i++){
		maze[i]=(char*)malloc(sizeof(char)*size_x);
			for(j=0;j<size_x;j++)
				maze[i][j]='#';
	}
	...
	while(maze[i=rand()%size_y][1]=='#'){
		printf("%c",maze[i][1]);
	}
	while(maze[j=rand()%size_y][size_x-2]=='#'){
		printf("%c",maze[j][size_x-2]);
	}
...
void left_hand_rule(char *maze[size_x],int x,int y,direction_list* d1){
...
direction_list* turn_left(char *maze[size_x],int* x,int* y,direction_list* d1){
	...
	else if (strcmp(d1->name,"west")==0 && *y+1<size_y && maze[*y+1][*x]!='#'){
		d1=d1->next;
		*y+=1;
	}
	else if (strcmp(d1->name,"south")==0 && *x+1<size_x && maze[*y][*x+1]!='#'){
		d1=d1->next;
		*x+=1;
	}
...
int go_straight(char *maze[size_x],int* x,int* y,direction_list* d1){
	
		int first_x=*x;
		int first_y=*y;
		
	if(strcmp(d1->name,"east")==0 && *x+1<size_x && maze[*y][*x+1]!='#')  // if our direction is east what we need for turn left x+1 ,y 
		*x+=1;																		// do other with same logic
	else if(strcmp(d1->name,"north")==0 && *y-1>=0 && maze[*y-1][*x]!='#')
		*y-=1;
	else if(strcmp(d1->name,"west")==0 && *x-1>=0 && maze[*y][*x-1]!='#')
		*x-=1;
	else if(strcmp(d1->name,"south")==0 && *y+1<size_y && maze[*y+1][*x]!='#')
		*y+=1;
...
direction_list* turn_right(char *maze[size_x],int* x,int* y,direction_list* d1){
	
	direction_list* iter=d1;
	
	if(strcmp(d1->name,"east")==0 && *y+1<size_y && maze[*y+1][*x]!='#'){ // if our direction is east what we need for turn right x ,y+1 
		d1=d1->prev;																// do other with same logic
		*y+=1;		
	}
	else if(strcmp(d1->name,"north")==0 && *x+1<size_x && maze[*y][*x+1]!='#'){
		d1=d1->prev;
		*x+=1;
	}
