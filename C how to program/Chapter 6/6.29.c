/*
	...
	Date:		  10.08.20 14:36
	Description:  (Knight’s Tour: Closed Tour Test) In the Knight’s Tour, a full tour occurs when the knight
					makes 64 moves touching each square of the chessboard once and only once. A closed tour occurs
					when the 64th move is one move away from the location in which the knight started the tour. 
					Modify the Knight’s Tour program you wrote in Exercise 6.24 to test for a closed tour if a full tour has
					occurred.
*/

	// code is same with 6.24_d.c I just added between 67-70
...
int main(void){
	...
	start_position[0]=currentRow;
	start_position[1]=currentColumn;
	...
	if(fabs(start_position[0]-currentRow)<=1 && fabs(start_position[1]-currentColumn)<=1)  //   control it is full tour or not
		puts("\nIt is Full tour");
	else 
		puts("\nNot a full tour");
