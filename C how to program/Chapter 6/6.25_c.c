...
//	puts("");
	...
	printf("%s%d%s","Arived 64 moves with ",j,".th tour");
...
int play_horse(int board[][8],int const horizontal[],int const vertical[]){
	...
	if( currentRow + vertical[i] <8 && currentRow + vertical[i] >=0 && currentColumn + horizontal[i]<8 && currentColumn + horizontal[i]>=0) //check the board limits
	if(board[currentRow + vertical[i]][currentColumn + horizontal[i]] != 1){	// and check the previous movements
